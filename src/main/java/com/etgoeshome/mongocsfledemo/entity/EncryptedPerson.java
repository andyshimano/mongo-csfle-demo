...
package com.etgoeshome.mongocsfledemo.entity;
...
public class EncryptedPerson {
	...
	public BsonBinary getSsn() {
		if(ssn.getType() != 6)			 
			ssn = new BsonBinary((byte)6, ssn.getData());
		return ssn;
	}
	...
	public BsonBinary getPhone() {
		if(phone.getType() != 6)			 
			phone = new BsonBinary((byte)6, phone.getData());
		return phone;
	}
	...
	public BsonBinary getBloodType() {
		if(bloodType.getType() != 6)			 
			bloodType = new BsonBinary((byte)6, bloodType.getData());		
		return bloodType;
	}
